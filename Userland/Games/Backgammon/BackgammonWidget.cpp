...
void BackgammonWidget::keydown_event(GUI::KeyEvent& event)
{
    set_override_cursor(Gfx::StandardCursor::None);
    switch (event.key()) {
...
void BackgammonWidget::set_piece_set(StringView set)
{
...
Optional<Chess::Square> BackgammonWidget::mouse_to_square(GUI::MouseEvent& event) const
{
    int const min_size = min(width(), height());
    int const widget_offset_x = (window()->width() - min_size) / 2;
    int const widget_offset_y = (window()->height() - min_size) / 2;
...
RefPtr<Gfx::Bitmap const> BackgammonWidget::get_piece_graphic(Chess::Piece const& piece) const
{
...
void BackgammonWidget::set_board_theme(StringView name)
{
...
bool BackgammonWidget::want_engine_move()
{
    if (!m_engine)
        return false;
...
void BackgammonWidget::input_engine_move()
{
    if (!want_engine_move())
        return;
...
void BackgammonWidget::playback_move(PlaybackDirection direction)
{
...
ErrorOr<String> BackgammonWidget::get_fen() const
{
    return TRY(m_playback ? m_board_playback.to_fen() : m_board.to_fen());
}

ErrorOr<void> BackgammonWidget::import_pgn(Core::File& file)
{
    m_board = Chess::Board();
...
ErrorOr<void> BackgammonWidget::export_pgn(Core::File& file) const
{
    // Tag Pair Section
...
void BackgammonWidget::flip_board()
{
    if (want_engine_move()) {
...
bool BackgammonWidget::check_game_over(ClaimDrawBehavior claim_draw_behavior)
{
...
void BackgammonWidget::config_string_did_change(StringView domain, StringView group, StringView key, StringView value)
{
...
void BackgammonWidget::config_bool_did_change(StringView domain, StringView group, StringView key, bool value)
{
    if (domain != "Games"sv && group != "Chess"sv)
        return;
